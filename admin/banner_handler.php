...
// --- LÓGICA DE EDIÇÃO CORRIGIDA E ROBUSTA ---
if ($action === 'edit_banner' && isset($_POST['banner_id'])) {
    $id = (int)$_POST['banner_id']; // Força o ID a ser um inteiro para segurança
    
    $title = !empty(trim($_POST['title'])) ? trim($_POST['title']) : $default_title;
    $subtitle = !empty(trim($_POST['subtitle'])) ? trim($_POST['subtitle']) : $default_subtitle;

    $stmt = $pdo->prepare("UPDATE hero_banners SET title = :title, subtitle = :subtitle WHERE id = :id");
